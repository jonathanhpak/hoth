...
function makeCursorURI(svgContent) {
  return `url("data:image/svg+xml,${encodeURIComponent(`<svg xmlns='http://www.w3.org/2000/svg' width='24' height='24' viewBox='0 0 24 24' shape-rendering='crispEdges'>${svgContent}</svg>`)}")`;
}
...
const CURSORS = [
  ...
  },
  {
    id: "sword",
    name: "IRON BLADE",
    desc: "A trusty sword. Point and strike.",
    price: 60,
    icon: "⚔",
    tags: ["WEAPON","COMMON"],
    hotspot: "2 2",
    svgBody: `
      <rect x='2'  y='2'  width='2' height='2' fill='#c8c8c8'/>
      <rect x='4'  y='4'  width='2' height='2' fill='#d8d8d8'/>
      <rect x='6'  y='6'  width='2' height='2' fill='#e8e8e8'/>
      <rect x='8'  y='8'  width='2' height='2' fill='#ffffff'/>
      <rect x='10' y='10' width='2' height='2' fill='#e0e0e0'/>
      <rect x='12' y='12' width='2' height='2' fill='#c8c8c8'/>
      <rect x='14' y='14' width='2' height='2' fill='#b0b0b0'/>
      <rect x='3'  y='2'  width='1' height='1' fill='#a0a0a0'/>
      <rect x='2'  y='3'  width='1' height='1' fill='#a0a0a0'/>
      <rect x='6'  y='4'  width='2' height='1' fill='#888'/>
      <rect x='4'  y='6'  width='1' height='2' fill='#888'/>
      <rect x='2'  y='16' width='4' height='2' fill='#8b5a1a'/>
      <rect x='1'  y='15' width='2' height='4' fill='#6b3a0a'/>
    `,
    preview: `
      <rect x='2'  y='2'  width='2' height='2' fill='#c8c8c8'/>
      <rect x='4'  y='4'  width='2' height='2' fill='#d8d8d8'/>
  ...
  },
  {
    ...
    hotspot: "4 4",
    svgBody: `
      <rect x='4'  y='2'  width='2' height='14' fill='#445566'/>
      <rect x='5'  y='2'  width='1' height='14' fill='#667788'/>
      <rect x='4'  y='2'  width='2' height='4'  fill='#aabbcc'/>
      <rect x='5'  y='2'  width='1' height='4'  fill='#ccdde8'/>
      <rect x='4'  y='16' width='2' height='2'  fill='#334455'/>
      <rect x='2'  y='10' width='6' height='2'  fill='#556677'/>
      <rect x='3'  y='9'  width='4' height='1'  fill='#778899'/>
      <rect x='4'  y='18' width='2' height='4'  fill='#6b3a0a'/>
      <rect x='3'  y='19' width='4' height='3'  fill='#8b5a2a'/>
      <rect x='8'  y='4'  width='2' height='2'  fill='#224466' opacity='0.5'/>
      <rect x='2'  y='6'  width='2' height='2'  fill='#334455' opacity='0.4'/>
    `,
    preview: `
      <rect x='4'  y='2'  width='2' height='14' fill='#445566'/>
      <rect x='5'  y='2'  width='1' height='14' fill='#667788'/>
...
const GEAR_COMMON = [
  ...
  { id:"g_wood_shield",   name:"WOODEN SHIELD",  slot:"shield", rarity:"common", icon:"🛡",  stats:{ def:3 },         desc:"Splintery but sturdy enough." },
...
export default function App() {
    ...
    async function load() {
      try {
        const [tR, qR, cR, invR, eqR, acR, gtR, cpR, ucR, opR, thR, otR, acurR, ocurR] = await Promise.allSettled([
          ...
          window.storage.get("ownedCursors-v1"),
        ]);
  ...
  useEffect(() => { if (!loaded) return; window.storage.set("ownedCursors-v1", JSON.stringify(ownedCursors)).catch(()=>{}); }, [ownedCursors, loaded]);
  ...
  const openChest = () => {
    ...
    setTimeout(() => {
      ...
      if (result.type === "character") {
        ...
        const alreadyOwned = inventory.some(i => i.id.split("_")[0] === result.id);
        if (alreadyOwned) {
          const coins = DUPE_COINS[result.rarity] ?? 10;
          setChar(p => ({ ...p, coins: p.coins + coins }));
          result = { ...result, duplicate: true, dupCoins: coins };
  ...
  const unlockAll = () => {
    ...
    setOwnedThemes(THEMES.map(t => t.id));
    setOwnedCursors(CURSORS.map(c => c.id));
    setTp(99999);
    setChar(p => ({ ...p, coins: 99999 }));
    setCharProgression(prev => {
      const next = { ...prev };
      CHARACTERS_CATALOG.forEach(c => {
        if (!next[c.id]) next[c.id] = { ...baseChar(), maxHp:c.baseHp, hp:c.baseHp, atk:c.baseAtk, def:c.baseDef };
      });
      return next;
    });
  };
